...
class Payment {
  ...
  static async handleKoaRequest(ctx, self, type) {
    ctx.reply = msg => {
    ...
    ctx.replyNative = (prepay_id, err_code_des) => {
      ctx.type = "application/xml; charset=utf-8";
      ctx.body = self._getNativeReply(prepay_id, err_code_des);
    };

    try {
      if (typeof ctx.request.body !== "string") throw new Error("XMLDataError");
      ctx.request.weixin = await self._parse(
        ctx.request.body,
        "middleware_" + type
      );
    } catch (err) {
  ...
  middleware(type = "pay") {
    return async (ctx, next) => {
      await Payment.handleKoaRequest(ctx, this, type);
      await next();
    };
  }
